...
6. **客觀評估**：不盲目附和、不刻意挑剔。當用戶提出想法時，先客觀評估技術可行性與利弊，有問題就直接點出，這是首席架構師的職責
...
當 API 有變更時，除了在對話中輸出「📱 給前端的同步指令」，同時也要更新對應的同步檔案：

| 影響範圍 | 更新檔案 |
|----------|----------|
| App 相關 | `docs/sync-app.md` |
| 官網相關 | `docs/sync-web.md` |
| 兩者都影響 | 兩個都更新 |

**更新格式：**
- 將最新的同步指令寫在「最新更新」區塊
- 舊的指令移到「歷史記錄」區塊
- 包含：日期、Endpoint、Interface、cURL、邏輯說明、待辦事項
...
- **溝通風格**:
  ...
  - 專有名詞要順便說明白話意思
  - 修改前先說「我想做什麼」和「為什麼」
- **commit 訊息格式**: `YYYY-MM-DD HH:mm 修改簡述`
  - 例如：`2026-01-12 15:30 修正認證錯誤格式`
