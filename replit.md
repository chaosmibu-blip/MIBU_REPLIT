...
【後端更新摘要】
新增全域排除功能，讓管理員可以將不適合的地點（如旅行社、政府機關）永久排除，所有使用者都不會抽到這些地點。
place_feedback 表現在支援 userId = null 代表全域排除。

【API 變更】
- 新增 API：GET /api/admin/global-exclusions - 取得全域排除清單
- 新增 API：POST /api/admin/global-exclusions - 新增全域排除地點
- 新增 API：DELETE /api/admin/global-exclusions/:id - 移除全域排除地點

【資料結構變更】
place_feedback 表：
- userId = null 時代表全域排除（所有使用者都不會抽到）
- userId = 某使用者 時代表個人排除

【前端需要的改動】
1. 如果有管理員後台，可新增「全域排除管理」頁面
2. 抽卡邏輯不需修改，後端會自動過濾全域排除的地點

【API 呼叫範例】

// 取得全域排除清單
const response = await fetch('https://gacha-travel--s8869420.replit.app/api/admin/global-exclusions', {
  method: 'GET',
  headers: {
...
// 移除全域排除
const response = await fetch('https://gacha-travel--s8869420.replit.app/api/admin/global-exclusions/123', {
  method: 'DELETE',
  headers: {
    'Authorization': `Bearer ${token}`
  }
...
{
  success: true;
  exclusion: { id, userId, placeName, district, city, penaltyScore, createdAt }
}

// DELETE 回傳
{
  success: true;
  message: "Global exclusion removed"
}
