...
| 功能 | App 中是否實作 | 說明 |
|------|---------------|------|
| 策劃師服務購買 | ✅ 是 | 旅客線上付費購買策劃師服務 |
| 商家訂閱 | ❌ 否 | 僅在官方網站提供（iOS 跨平台服務規定） |

> **重要**：商家訂閱按鈕不得出現在 App 中，所有商家訂閱功能導向官方網站。
...
| `memory-auth-flow.md` | 認證流程 | Apple/Google 登入、Token 管理、登出 |
| `memory-state.md` | 狀態管理 | Context、Zustand、快取策略 |
| `memory-assets.md` | 資源管理 | 圖片、字體、多語系 |
| `memory-specialist-payment.md` | 策劃師金流 | 旅客購買策劃師服務流程（App 唯一金流） |
