...
export const SideNav: React.FC<SideNavProps> = ({ currentView, onChange, language }) => {
  ...
  return (
    <>
      ...
      <nav className="hidden md:flex fixed right-0 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-md border-l border-slate-100 shadow-lg rounded-l-2xl z-40 py-4 px-2">
        <div className="flex flex-col items-center gap-2">
          ...
          <button
             ...
             className="flex flex-col items-center justify-center p-2 group"
             data-testid="nav-merchant-desktop"
             title={t.navStore}
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
            ...
            <span className={`text-[10px] font-medium mt-1 whitespace-nowrap ${currentView.startsWith('merchant') ? 'text-indigo-600' : 'text-slate-300'}`}>
              {t.navStore}
            </span>
          </button>
        </div>
      ...
      {/* Mobile: Bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 pb-safe-bottom z-40 max-w-md mx-auto">
