          ...
          {previewResults.map((result, idx) => (
            <div key={idx} className="border border-slate-200 rounded-xl overflow-hidden">
              <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                <span className="font-medium text-slate-700">{result.keyword}</span>
                <span className="text-sm text-slate-500">{result.places.length} 筆</span>
              </div>
              <div className="divide-y divide-slate-100 max-h-60 overflow-y-auto">
                {result.places.slice(0, 5).map((place, pIdx) => (
                  <div key={pIdx} className="px-4 py-2 flex items-center justify-between">
                    <div>
                      <div className="font-medium text-slate-800 text-sm">{place.placeName}</div>
                      <div className="text-xs text-slate-500">{place.address}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-amber-600">★ {place.rating || '-'}</span>
                      <span className="text-xs px-2 py-0.5 bg-slate-100 rounded text-slate-600">{place.category}</span>
                    </div>
                  </div>
                ))}
                {result.places.length > 5 && (
                  <div className="px-4 py-2 text-center text-sm text-slate-500">
                    還有 {result.places.length - 5} 筆...
                  </div>
                )}
              </div>
            </div>
