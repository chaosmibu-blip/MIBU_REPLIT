...
### 2026-01-17 #006：🎉 活動系統 API（首頁活動展示）

**變更類型**: 🟢 新增功能

**背景說明**:
後端已完成活動系統，支援首頁展示公告、節慶活動、限時活動三種類型。

**記憶庫**: `docs/memory-events.md`
...
**新增檔案**: `src/services/eventService.ts`
...
export const eventService = {
  ...
  async getEvents(params?: {
    type?: 'announcement' | 'festival' | 'limited';
    city?: string;
    limit?: number;
    offset?: number;
  ...
  // 取得活動詳情
  async getEvent(id: number): Promise<{ success: boolean; event: Event }> {
...
**整合位置**: 首頁（`app/(tabs)/index.tsx` 或類似）
...
| 活動標題 | `event.title` |
| 活動圖片 | `event.imageUrl` |
| 活動描述 | `event.description` |
| 活動日期 | `event.startDate` ~ `event.endDate` |
| 活動地點 | `event.location` |
| 查看更多按鈕 | 開啟 `event.sourceUrl` |
...
### 2026-01-17 #005：🚨 UI 畫面實作（審計發現缺失）

**變更類型**: 🔴 重大缺失（UI 畫面未實作）

**背景說明**:
經三端審計發現，APP 已完成 Types + Services 層，但**缺少對應的 UI 畫面**。
- ✅ `src/types/` - 4 個類型檔案已建立
- ✅ `src/services/` - 4 個 API 服務已建立
- ❌ `src/screens/` 或 `app/` - 無對應畫面

**審計來源**: 後端 SYNC_QUEUE.md #005
...
**新增檔案**:
- `src/screens/EconomyScreen.tsx` 或 `app/(tabs)/economy.tsx`

**功能需求**:

| 元素 | API | 說明 |
|------|-----|------|
| 等級卡片 | `GET /api/user/level` | 顯示等級、經驗值進度條 |
| 經驗值歷史 | `GET /api/user/experience/history` | 歷史記錄列表 |
| 成就徽章牆 | `GET /api/user/achievements` | 成就卡片網格 |
| 領取按鈕 | `POST /api/user/achievements/:id/claim` | 未領取成就的「領取」按鈕 |
...
**新增檔案**:
- `src/screens/CrowdfundScreen.tsx` 或 `app/crowdfund/index.tsx`
- `src/screens/CrowdfundDetailScreen.tsx` 或 `app/crowdfund/[id].tsx`

**功能需求**:

| 元素 | API | 說明 |
|------|-----|------|
| 活動列表 | `GET /api/crowdfund/campaigns` | 進行中/即將開始活動 |
| 活動詳情 | `GET /api/crowdfund/campaigns/:id` | 進度、贊助者列表 |
| IAP 贊助 | `POST /api/crowdfund/contribute` | iOS/Android IAP 串接 |
| 我的贊助 | `GET /api/crowdfund/my-contributions` | 個人贊助記錄 |

##### 3. 推薦系統畫面（Phase 3）

**新增檔案**:
- `src/screens/ReferralScreen.tsx` 或 `app/referral/index.tsx`

**功能需求**:

| 元素 | API | 說明 |
|------|-----|------|
| 我的推薦碼 | `GET /api/referral/my-code` | 推薦碼 + 複製/分享按鈕 |
| 輸入推薦碼 | `POST /api/referral/apply` | 輸入框 + 套用按鈕 |
| 推薦清單 | `GET /api/referral/my-referrals` | 我推薦的人列表 |
| 獎勵餘額 | `GET /api/referral/balance` | 餘額顯示 |
| 申請提現 | `POST /api/referral/withdraw` | 提現表單 |

##### 4. 用戶貢獻畫面（Phase 4）

**新增檔案**:
- `src/screens/ContributionScreen.tsx` 或 `app/contribution/index.tsx`

**功能需求**:

| 元素 | API | 說明 |
|------|-----|------|
| 回報歇業 | `POST /api/contribution/report-closed` | 景點詳情頁按鈕 |
| 建議景點 | `POST /api/contribution/suggest-place` | 新景點表單 |
| 社群投票 | `GET /api/contribution/pending-votes` | 待投票列表 |
| 投票按鈕 | `POST /api/contribution/vote/:placeId` | 贊成/反對 |

##### 5. 帳號綁定畫面（Phase 5）

**新增檔案**:
- `src/screens/AccountScreen.tsx` 或 `app/account/index.tsx`

**功能需求**:

| 元素 | API | 說明 |
|------|-----|------|
| 已綁定帳號 | `GET /api/auth/linked-accounts` | Apple/Google/Line 列表 |
| 綁定新帳號 | `POST /api/auth/link` | 各平台 OAuth 按鈕 |
| 解除綁定 | `DELETE /api/auth/unlink/:provider` | 各帳號旁的解除按鈕 |
...
│   ├── index.tsx         # 首頁
│   ├── gacha/            # 扭蛋（已有）
│   ├── collection.tsx    # 收藏（已有）
│   ├── planner/          # 策畫（已有）
│   ├── settings.tsx      # 設定（已有）
│   └── profile/          # 👈 建議新增
│       ├── index.tsx     # 個人資料首頁
│       ├── economy.tsx   # 經濟系統（等級/成就）
│       ├── referral.tsx  # 推薦系統
│       └── account.tsx   # 帳號綁定
├── crowdfund/            # 👈 建議新增
│   ├── index.tsx         # 募資列表
...
1. **高優先級**：經濟系統（用戶留存核心）、帳號綁定（安全性）
2. **中優先級**：推薦系統（成長引擎）
3. **低優先級**：募資系統、用戶貢獻（可後續迭代）
