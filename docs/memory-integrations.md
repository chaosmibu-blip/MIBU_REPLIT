...
#### Text Search (New) SKU 層級

| SKU | 費用/千次（美金） | 費用/千次（台幣） | 包含欄位 |
|-----|-----------------|-----------------|---------|
| **ID Only** | ~$0-5 | ~$0-157 | id, displayName |
| **Essentials** | ~$5-10 | ~$157-314 | + location, address |
| **Pro** | $32 | NT$1,004 | + rating, types, primaryType |
| **+ Contact Data** | +$3 | +NT$94 | + openingHours |

#### Place Details SKU 層級

| SKU | 費用/千次（美金） | 費用/千次（台幣） |
|-----|-----------------|-----------------|
| Basic | $5-10 | NT$157-314 |
| Advanced | $17-20 | NT$534-628 |

### 費用優化策略（2026-01-01 新增）

#### 已實施
1. **移除 openingHours 欄位**：採集時不請求營業時間，節省 Contact Data SKU（+$3/千次）
2. **智慧時段推斷**：改用 AI 根據類別/子類別推斷營業時段

#### 評估但未採用：分段式採集（Two-Stage Fetching）

| 方案 | 概念 | 結論 |
|------|------|------|
| **分段式** | 第一步只取 ID → 本地去重 → 第三步對新 ID 取詳情 | ⚠️ 不建議 |

**不採用原因**：
- Text Search 需要 location/address 驗證城市，無法只取 ID
- Place Details ($17/千次) × 新資料筆數 可能超過 Text Search Pro ($32/千次)
- 只有重複率 > 80% 時才有優勢，但實際重複率約 30-50%

#### 有效降低費用的方法

| 方法 | 效果 | 風險 |
|------|------|------|
| **減少關鍵字數量** | 10 → 5 個/類別 | 可能漏掉好景點 |
| **減少分頁數** | 3 頁 → 1 頁 | 減少重複但也減少資料量 |
| **移除 openingHours** | 節省 $3/千次 | ✅ 已實施 |
| **採集前查詢資料庫** | 避免已存在城市重複採集 | 手動管理 |
