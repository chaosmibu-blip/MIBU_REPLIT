...
└──────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌──────────────────────────────────────────────────────────────┐
│  Phase 3: 三層備援（若仍不足）🆕 2026-01-08                  │
├──────────────────────────────────────────────────────────────┤
│  9a. 第一層: 從同區任意類別補足（仍排除去重）                 │
│  9b. 第二層: 從全城市補足                                    │
│  9c. 第三層: 完全忽略去重，從同區抽取未選過的景點             │
└──────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌──────────────────────────────────────────────────────────────┐
│  Phase 4: 三輪 AI 驗證排序（2026-01-08 恢復）                 │
├──────────────────────────────────────────────────────────────┤
│  10. 經緯度排序: 最近鄰居演算法，從最北點開始連接最近點       │
│  11. 時段排序: 根據營業時間/類別推斷最佳時段                  │
│  12. 第一輪 AI（Gemini）:                                    │
│      - 輸入：名稱、類別、子類別、描述、營業時間               │
│      - 輸出：order + reject + reason                         │
│      - 可排除不適合景點（歇業、非旅遊點、重複）               │
│  13. 若第一輪有排除 → 補抽 → 第二輪 AI:                      │
│      - 重新排序，仍可排除                                    │
│  14. 若第二輪有排除 → 補抽 → 第三輪 AI:                      │
