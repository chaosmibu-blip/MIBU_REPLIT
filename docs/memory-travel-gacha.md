...
3. **結構化選點**（保底 + 等權重隨機）:
   ...
   - **美食上限**：不超過總數一半
   - **住宿**：≥9張才有，最多 1 個
   - **剩餘額度**：六大類別（美食、景點、購物、娛樂設施、生態文化教育、遊程體驗）等權重隨機分配
4. **圖鑑去重保護**: 查詢用戶圖鑑最近 36 張，完全排除這些地點
5. **經緯度排序**: 最近鄰居演算法優化動線
6. **智慧時段排序**: 根據營業時間/類別推斷最佳時段
7. **AI 智慧排序**（權重最高）: Gemini 負責最終行程順序（2026-01-02 更新）
   - 模型：優先使用 `gemini-3-flash-preview`（Google 直連），備用 `gemini-2.5-flash`
   - 傳送完整資訊：名稱、類別、子類別、**經緯度**、描述（80字）、營業時間
   - Prompt 格式：`{idx}. {name}｜{category}/{subcategory}｜({lat},{lng})｜{description}｜營業:{hours}`
   - AI 回傳 `{"order":[...], "reason":"...", "reject":[...]}`
   - 被拒絕的地點會從備用池補充，然後重新送 AI 審核（最多一輪）
   - 排序理由記錄在 log 中供開發者檢查（不回傳給前端）
