...
4. **三層備援機制** 🆕 2026-01-08 v2:
   - **第一層**：類別耗盡時，從同區任意類別補足（仍排除去重）
   - **第二層**：同區不足時，從全城市補足
   - **第三層**（絕對模式）：完全忽略去重，從同區抽取未選過的景點
5. **圖鑑去重保護**: 查詢用戶圖鑑最近 36 張，完全排除這些地點
6. **經緯度排序**: 最近鄰居演算法優化動線
7. **智慧時段排序**: 根據營業時間/類別推斷最佳時段
8. **AI 智慧排序**（權重最高）: Gemini 負責最終行程順序
   - 傳送完整資訊（名稱、類別、子類別、經緯度、描述、營業時間）
   - AI 可回傳 X:編號 標記不適合的地點
   - 確保美食不連續超過 2 個
9. **AI 日誌記錄**: 儲存完整的排序過程到 `gacha_ai_logs` 表
10. **張數不足提示**: 當實際抽出 < 請求張數時，回傳 `isShortfall: true`
